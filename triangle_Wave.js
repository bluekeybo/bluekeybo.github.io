var PHASOR = (function() {
    ...
    var sine = d3.svg.line()
		...
		{			
			valReturned = valReturned+Math.pow(-1,count)*(1/Math.pow(2*count+1,2))*Math.sin((2*count+1)*2*Math.PI*d * 0.5)  * GET_PHASOR_AMPLITUDE();	
			oddNum = oddNum+2;
			counter = counter + 1;
			}		            
			count = 0;
			oddNum = 1;
			return yRange(1.2*(8/(Math.pow(Math.PI,2)))*valReturned);
        });
